...
public class RegexEditText extends AppCompatEditText implements InputFilter {
    ...
    public RegexEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.public_RegexEditText);

        if (array.hasValue(R.styleable.public_RegexEditText_public_inputRegex)) {
            setInputRegex(array.getString(R.styleable.public_RegexEditText_public_inputRegex));
        } else {
            if (array.hasValue(R.styleable.public_RegexEditText_public_regexType)) {
                int regexType = array.getInt(R.styleable.public_RegexEditText_public_regexType, 0);
                switch (regexType) {
                    case 0x01:
