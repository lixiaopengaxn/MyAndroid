...
public final class SettingBar extends FrameLayout {
    ...
    public SettingBar(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.public_SettingBar);

        // 文本设置
        if (array.hasValue(R.styleable.public_SettingBar_public_bar_leftText)) {
            setLeftText(array.getString(R.styleable.public_SettingBar_public_bar_leftText));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_rightText)) {
            setRightText(array.getString(R.styleable.public_SettingBar_public_bar_rightText));
        }

        // 提示设置
        if (array.hasValue(R.styleable.public_SettingBar_public_bar_leftHint)) {
            setLeftHint(array.getString(R.styleable.public_SettingBar_public_bar_leftHint));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_rightHint)) {
            setRightHint(array.getString(R.styleable.public_SettingBar_public_bar_rightHint));
        }

        // 图标设置
        if (array.hasValue(R.styleable.public_SettingBar_public_bar_leftIcon)) {
            setLeftIcon(array.getDrawable(R.styleable.public_SettingBar_public_bar_leftIcon));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_rightIcon)) {
            setRightIcon(array.getDrawable(R.styleable.public_SettingBar_public_bar_rightIcon));
        }

        // 文字颜色设置
        setLeftColor(array.getColor(R.styleable.public_SettingBar_public_bar_leftColor, ContextCompat.getColor(getContext(), R.color.public_black80)));
        setRightColor(array.getColor(R.styleable.public_SettingBar_public_bar_rightColor, ContextCompat.getColor(getContext(), R.color.public_black60)));

        // 文字大小设置
        setLeftSize(TypedValue.COMPLEX_UNIT_SP, array.getDimensionPixelSize(R.styleable.public_SettingBar_public_bar_leftSize, 15));
        setRightSize(TypedValue.COMPLEX_UNIT_SP, array.getDimensionPixelSize(R.styleable.public_SettingBar_public_bar_rightSize, 14));

        // 分割线设置
        if (array.hasValue(R.styleable.public_SettingBar_public_bar_lineColor)) {
            setLineDrawable(array.getDrawable(R.styleable.public_SettingBar_public_bar_lineColor));
        } else {
            setLineDrawable(new ColorDrawable(0xFFECECEC));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_lineVisible)) {
            setLineVisible(array.getBoolean(R.styleable.public_SettingBar_public_bar_lineVisible, true));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_lineSize)) {
            setLineSize(array.getDimensionPixelSize(R.styleable.public_SettingBar_public_bar_lineSize, 0));
        }

        if (array.hasValue(R.styleable.public_SettingBar_public_bar_lineMargin)) {
            setLineMargin(array.getDimensionPixelSize(R.styleable.public_SettingBar_public_bar_lineMargin, 0));
        }

        if (getBackground() == null) {
            StateListDrawable drawable = new StateListDrawable();
            drawable.addState(new int[]{android.R.attr.state_pressed}, new ColorDrawable(ContextCompat.getColor(getContext(), R.color.public_black5)));
            drawable.addState(new int[]{android.R.attr.state_selected}, new ColorDrawable(ContextCompat.getColor(getContext(), R.color.public_black5)));
            drawable.addState(new int[]{android.R.attr.state_focused}, new ColorDrawable(ContextCompat.getColor(getContext(), R.color.public_black5)));
            drawable.addState(new int[]{}, new ColorDrawable(ContextCompat.getColor(getContext(), R.color.public_white)));
