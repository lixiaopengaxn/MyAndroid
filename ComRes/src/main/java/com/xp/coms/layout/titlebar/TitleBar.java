package com.xp.coms.layout.titlebar;
...
import com.xp.coms.layout.titlebar.initializer.BaseBarInitializer;
import com.xp.coms.layout.titlebar.initializer.LightBarInitializer;
import com.xp.coms.layout.titlebar.initializer.NightBarInitializer;
import com.xp.coms.layout.titlebar.initializer.RippleBarInitializer;
import com.xp.coms.layout.titlebar.initializer.TransparentBarInitializer;
...
public class TitleBar extends FrameLayout
    ...
    public TitleBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.public_TitleBar);

        // 标题栏样式设置
        switch (array.getInt(R.styleable.public_TitleBar_public_barStyle, 0)) {
            case 0x10:
        ...
        if (array.hasValue(R.styleable.public_TitleBar_public_drawableSize)) {
            setDrawableSize(array.getDimensionPixelSize(R.styleable.public_TitleBar_public_drawableSize, 0));
        }

        // 设置文字和图标之间的间距
        if (array.hasValue(R.styleable.public_TitleBar_android_drawablePadding)) {
            setDrawablePadding(array.getDimensionPixelSize(R.styleable.public_TitleBar_android_drawablePadding, 0));
        }

        // 标题设置
        if (array.hasValue(R.styleable.public_TitleBar_public_leftTitle)) {
            setLeftTitle(array.getString(R.styleable.public_TitleBar_public_leftTitle));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_title)) {
            setTitle(array.getString(R.styleable.public_TitleBar_public_title));
        } else {
        ...
        if (array.hasValue(R.styleable.public_TitleBar_public_rightTitle)) {
            setRightTitle(array.getString(R.styleable.public_TitleBar_public_rightTitle));
        }

        // 图标设置
        if (array.hasValue(R.styleable.public_TitleBar_public_leftIcon)) {
            setLeftIcon(BaseBarInitializer.getDrawableResources(getContext(), array.getResourceId(R.styleable.public_TitleBar_public_leftIcon, 0)));
        } else {
            if (!array.getBoolean(R.styleable.public_TitleBar_public_backButton, true)) {
                // 不显示返回图标
                setLeftIcon(null);
            }
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_rightIcon)) {
            setRightIcon(BaseBarInitializer.getDrawableResources(getContext(), array.getResourceId(R.styleable.public_TitleBar_public_rightIcon, 0)));
        }

        // 文字颜色设置
        if (array.hasValue(R.styleable.public_TitleBar_public_leftColor)) {
            setLeftColor(array.getColor(R.styleable.public_TitleBar_public_leftColor, 0));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_titleColor)) {
            setTitleColor(array.getColor(R.styleable.public_TitleBar_public_titleColor, 0));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_rightColor)) {
            setRightColor(array.getColor(R.styleable.public_TitleBar_public_rightColor, 0));
        }

        // 文字大小设置
        if (array.hasValue(R.styleable.public_TitleBar_public_leftSize)) {
            setLeftSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.public_TitleBar_public_leftSize, 0));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_titleSize)) {
            setTitleSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.public_TitleBar_public_titleSize, 0));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_rightSize)) {
            setRightSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.public_TitleBar_public_rightSize, 0));
        }

        // 背景设置
        if (array.hasValue(R.styleable.public_TitleBar_public_leftBackground)) {
            setLeftBackground(array.getDrawable(R.styleable.public_TitleBar_public_leftBackground));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_rightBackground)) {
            setRightBackground(array.getDrawable(R.styleable.public_TitleBar_public_rightBackground));
        }

        // 分割线设置
        if (array.hasValue(R.styleable.public_TitleBar_public_lineColor)) {
            setLineDrawable(array.getDrawable(R.styleable.public_TitleBar_public_lineColor));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_titleGravity)) {
            setTitleGravity(array.getInt(R.styleable.public_TitleBar_public_titleGravity, Gravity.NO_GRAVITY));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_titleStyle)) {
            setTitleStyle(Typeface.defaultFromStyle(array.getInt(R.styleable.public_TitleBar_public_titleStyle, Typeface.NORMAL)));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_lineVisible)) {
            setLineVisible(array.getBoolean(R.styleable.public_TitleBar_public_lineVisible, false));
        }

        if (array.hasValue(R.styleable.public_TitleBar_public_lineSize)) {
            setLineSize(array.getDimensionPixelSize(R.styleable.public_TitleBar_public_lineSize, 0));
        }

        // 如果设置了这两个属性，则将内间距置为空
        if (array.hasValue(R.styleable.public_TitleBar_android_paddingHorizontal) || array.hasValue(R.styleable.public_TitleBar_android_paddingVertical)) {
            setPadding(0, 0, 0, 0);
        }
        // 获取子 View 水平内间距
        mHorizontalPadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.public_TitleBar_android_paddingHorizontal,
                mCurrentInitializer.getHorizontalPadding(getContext())), getResources().getDisplayMetrics());
        // 获取子 View 垂直内间距
        mVerticalPadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.public_TitleBar_android_paddingVertical,
